...
window.loadConstants = async function (root = document) {
  ...
  const cssPromises = components.map(async (comp) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.warn(
          `[Framework] CSS não encontrado: ${cssPath}`
        )
  ...
  const jsPromises = components.map(async (comp) => {
    ...
    if (loadedJs.has(jsPath)) {
      if (window.Components?.[comp]?.init) {
        try {
          ...
          console.warn(
            `[Framework] Erro init componente ${comp}:`,
            err
          )
        }
      }
    ...
    try {
      ...
      if (!head.ok) {
        console.warn(
          `[Framework] Script não encontrado: ${jsPath}`
        )
        loadedJs.add(jsPath)
        return
      }
    ...
    await new Promise((resolve) => {
      ...
      script.onerror = () => {
        console.warn(
          `[Framework] Falha ao carregar script: ${jsPath}`
        )
        resolve()
      }
