...
window.Components["background-particles"] = (function () {
  ...
  function readConfig() {
    ...
    const count = parseInt(s.getPropertyValue("--particles-count")) || 60;
  ...
  function animate(ts) {
    ...
    for (const p of particles) {
      ...
      if (p.x <= 0) {
        p.x = 0;
        p.vx = Math.abs(p.vx) * (0.88 + Math.random() * 0.12);
        p.vy *= 0.98;
      ...
      if (p.y <= 0) {
        p.y = 0;
        p.vy = Math.abs(p.vy) * (0.88 + Math.random() * 0.12);
        p.vx *= 0.98;
      ...
      if (vmag === 0) {
        ...
        p.vx = Math.cos(a) * (minSpeed + Math.random() * minSpeed);
        p.vy = Math.sin(a) * (minSpeed + Math.random() * minSpeed);
  ...
  function init() {
    ...
    const count = Math.max(5, Math.round(config.count * areaFactor));

    particles = [];

    for (let i = 0; i < count; i++) {
      const size =
        Math.random() * (config.sizeMax - config.sizeMin) + config.sizeMin;
      createParticle(size);
    }
